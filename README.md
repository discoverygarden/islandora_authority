...
* `islandora_authority_solr_validate_field`: An optional Solr field to validate the contents of the element against; validation is done on both initial form load and element change, and a green checkmark is displayed if the element value matches a value in the given Solr field.
